...
import {addAdmin, signUp} from "../../../redux/actions/auth";
import AuthDetector from "../../auth/AuthDetector";
...
function AddAdminForm() {
    ...
    const message = useSelector((store) => store.auth.addAdminMessage);
    const status = useSelector((store) => store.auth.addAdminStatus);
    ...
    const handleSubmitForm = useCallback((ev) => {
        ...
        if (passwordStrength <= 60) {
            ...
            dispatch(addAdmin(authForm));
        }
    }, [dispatch, authForm]);

    return (<>
        <form className="auth-form-block add-admin" onSubmit={(ev) => handleSubmitForm(ev)}>
            ...
            <div className="auth-box auth-detector">
                <AuthDetector state={"admin"} getPasswordStrength={getPasswordStrength(authForm.password)}
                              confPassword={authForm.confPassword} passwordMatch={passwordMatch}/>
            </div>
            <Button variant={"auth-btn"}>Add Admin</Button>
        </form>
    </>);
}
